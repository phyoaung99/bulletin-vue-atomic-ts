  ...
  <FormTemplate>
    ...
    <template v-slot:form-body>
      <form class="form-wrap" @submit.prevent="onSubmit">
        ...
        <VFormControl>
          <VBtn :color="'primary'" submit>Create</VBtn>
          <VBtn
              :to="{ name: 'postList' }"
              :color="'secondary'"
              :class="'ms-3'"
  ...
  postCreate(postData.value)
    ...
    .catch((error: AxiosError) => {
      ...
      if (errRes?.status == 422) {
        if (errRes.data.errors?.title)
          titleError.value = errRes.data.errors?.title;
        if (errRes.data.errors?.description)
          descriptionError.value = errRes.data.errors?.description;
      } else {
